...
class StepWiseLR(tf.keras.optimizers.schedules.LearningRateSchedule):
    ...
    def __init__(
        ...
        shift_points=[0.2, 0.2, 0.2, 0.2, 0.05],
        scale_mode="cycle",
        final_lr_scale=1.0,
        name=None,
    ):
    ...
    def get_constant(self, learning_rate, step, step_size_partial, cycle):
    ...
    def __call__(self, step, optimizer=False):
        with tf.name_scope(self.name or "OneCycle"):
            ...
            alpha_factor = tf.cast(self.alpha_factor, dtype)
            ...
            #print(shift_steps)
            lr_segments = tf.map_fn(
                lambda x: self.get_constant(
                    initial_learning_rate * alpha_factor ** tf.cast(x, dtype),
                    normalized_steps[x],
                    shift_steps[x],
                    cycle
